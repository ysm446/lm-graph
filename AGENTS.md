...
UI に関わる作業の場合は、加えて `docs/design/style-guide.md` を必ず読む。

## UI スタイルガイド（UI 変更時は必読）

`docs/design/style-guide.md` は、このアプリの見た目に関する唯一の基準ドキュメントである。

色、タイポグラフィ、余白、角丸、影、ボタン、トグル、select、スライダー、セクション、メニュー、モーダル、ノードカード、ハンドル、エッジ、アイコン、アニメーションなど、ユーザーに見える部分を変更するときは、実装前に `docs/design/style-guide.md` を読み、その内容に従う。

- 色は `index.css` のデザイントークン（`var(--text)`、`var(--bg-card)`、`var(--accent)` など）を使う。新しい色を直書きしない。ノード種別の識別色だけが例外で、スタイルガイドの表に従う。
- フォントサイズ、余白、角丸、部品の class 構成は、スタイルガイドに記載された値から選ぶ。新しい値を勝手に作らない。
- 既存の共通部品（`IconButton`、`ToolbarButton`、`MenuAction`、`InspectorSection`、トグルスイッチ、`graph-slider`）を再利用し、見た目が似た別部品を新規に作らない。
- hover / 選択中 / 無効 / 既定値から変更あり / 生成中 の表現は、スタイルガイドの状態表現に合わせる。
- 作業の最後に、スタイルガイド末尾のチェックリストを確認する。
- どうしても新しいパターンが必要な場合は、`docs/design/style-guide.md` に追記し、同じ変更内で `更新日時` を更新する。
