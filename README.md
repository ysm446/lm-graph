...
- **選択テキスト校正** — ノード編集時と `Details` 編集時に、選択範囲から明示操作で校正案を呼び出し
...
- 長文の本文はここで全文表示します。
- タイトル行の右端に `Generate` と `Edit` ボタンがあります。
- タイトルや本文をダブルクリックすると編集モードに入れます。
- 生成中のストリーミング本文はここに表示されます。
- `text` ノードでは、編集中に選択した範囲から `校正` ボタンで校正ポップアップを開けます。
...
テキストを編集モードで選択すると、選択中に表示される `校正` ボタンからロード済みモデルへ校正リクエストを送れます。
...
- `Settings > Editing` から `Proofread from Selection` をオン / オフできます。
