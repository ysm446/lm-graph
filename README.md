...
- プロジェクト一覧・インライン名前変更・プロジェクト単位の保存アイコン
- `text` / `context` / `instruction` ノードによるグラフ編集
- 左右サイドバーのリサイズ
- 上流ノードをたどるコンテキスト収集
- ローカル `llama.cpp` を使ったストリーミング生成
- **生成キュー** — 生成中に別ノードの Generate を押すと順番待ちに入り、完了後に自動実行
- **ズームアウト時ノードタイトル表示** — 縮小時にノード外側へタイトルをフォント固定で表示
- **選択テキスト校正** — 編集中にテキストを選択すると LLM が自動で校正案を提示（Tab で適用）
...
- グラフ編集内容は、プロジェクト項目の保存アイコンを押すまでローカルの作業状態として保持されます。
- 未保存の変更がある場合は、アクティブなプロジェクト項目に紫のドットと保存アイコンが表示されます。
- 未保存のままプロジェクト切り替えやアプリ終了を行うと、警告が表示されます。
- サイドバー状態、ミニマップ表示、エッジスタイル、校正オン/オフなどの UI 設定は、Electron の `userData` 配下の JSON ファイルに保存されます。

## 校正機能

ノードを編集モードにしてテキストを選択すると、600ms 後に自動でロードされているモデルへ校正リクエストが送られます。

- 校正結果はポップアップにストリーミング表示されます。
- `Tab` または「適用」ボタンで選択範囲を校正後テキストに置換します。
- `Escape` または「キャンセル」で閉じます。
- 右サイドバーの **Editing → Proofread on Select** トグルでオン/オフできます。
