...
// Default selection priority: CUDA first.
...
export async function installLlamaVariant(input: {
  ...
  try {
    ...
    const serverZip = join(tmpdir(), `lm-graph-${randomUUID()}.zip`)
    ...
    if (variant.cudartUrl) {
      const cudartZip = join(tmpdir(), `lm-graph-${randomUUID()}.zip`)
...
function extractZip(zipPath: string, destDir: string): Promise<void> {
  // Windows 10/11 ship bsdtar, which extracts .zip archives.
